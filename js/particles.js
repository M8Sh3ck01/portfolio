...
class Particle {
    ...
    draw() {
        ...
        ctx.fillStyle = '#3b82f6'; // Match accent color (blue)
        ctx.fill();
    }
    ...
    update() {
        ...
        if (distance < mouse.radius + this.size) {
            if (mouse.x < this.x && this.x < canvas.width - this.size * 10) {
                this.x += 3; // Push particles away slightly
            }
...
function init() {
    particlesArray = [];
    // Number of particles - responsive density
    ...
    for (let a = 0; a < particlesArray.length; a++) {
        for (let b = a; b < particlesArray.length; b++) {
            ...
            if (distance < (canvas.width / 7) * (canvas.height / 7)) {
                opacityValue = 1 - (distance / 20000); // Fade out line as distance increases
                ctx.strokeStyle = 'rgba(59, 130, 246,' + (opacityValue * 0.15) + ')'; // Very subtle blue lines
                ctx.lineWidth = 1;
                ctx.beginPath();
